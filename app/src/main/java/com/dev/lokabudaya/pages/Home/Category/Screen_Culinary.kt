...
@Composable
fun CulinaryGridList() {
    LazyVerticalGrid(
        columns = GridCells.Fixed(2),
        contentPadding = PaddingValues(8.dp)
    ) {
        items(DataProvider.culinaryList.size) { index ->
            val culinaryItem = DataProvider.culinaryList[index]
            SearchCardItem(
                image = culinaryItem.imageRes,
                title = culinaryItem.title,
                price = culinaryItem.price,
                rating = culinaryItem.rating
            )
        }
    }
}